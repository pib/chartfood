...
if six.PY3:
    def csv_enc(s, _=None):
        return s

    def csv_dec(s, _=None):
        return s
else:
    def csv_enc(s, encoding="utf-8"):
        return s.encode(encoding)

    def csv_dec(s, encoding="utf-8"):
...
class Table(object):
    ...
    Create this object, populate it with data, then call one of the
    ToJS...  methods to return a string representation of the data in
    ...
    You can reorder the columns in the output table, and also specify
    row sorting order by column. The default column order is according
    to the original table_description parameter. Default row sort
    order is ascending, by column 1 values. For a dictionary, we sort
    the keys for order.

    The data and the table_description are closely tied, as described here:

    The table schema is defined in the class constructor's
    table_description parameter. The user defines each column using a
    tuple of (id[, type[, label[, custom_properties]]]). The default
    value for type is string, label is the same as ID if not
    specified, and custom properties is an empty dictionary if not
    specified.

    table_description is a dictionary or list, containing one or more
    column descriptor tuples, nested dictionaries, and lists. Each
    dictionary key, list element, or dictionary element must
    eventually be defined as a column description tuple. Here's an
    example of a dictionary where the key is a tuple, and the value is
    a list of two tuples:

        {('a', 'number'): [('b', 'number'), ('c', 'string')]}

    This flexibility in data entry enables you to build and manipulate
    your data in a Python structure that makes sense for your program.
    ...
    def __init__(self, table_description, data=None, custom_properties=None):
        ...
        See the class documentation for more information on table
        schema and data values.

        Args:

            table_description: A table schema, following one of the
                               formats described in
                               TableDescriptionParser(). Schemas
                               describe the column names, data types,
                               and labels. See
                               TableDescriptionParser() for acceptable
                               formats.

            data: Optional. If given, fills the table with the given
                  data. The data structure must be consistent with
                  schema in table_description. See the class
                  documentation for more information on acceptable
                  data. You can add data later by calling
                  AppendData().

            custom_properties: Optional. A dictionary from string to
                               string that goes into the table's
                               custom properties. This can be later
                               changed by changing
                               self.custom_properties.

        Raises:

            TableException: Raised if the data and the description did
            not match, or did not use the supported formats.
        """
    ...
    def CoerceValue(value, value_type):
        ...
        Args:
            ...
            value_type: One of "string", "number", "boolean", "date",
                        "datetime" or "timeofday".

        Returns:

            An item of the Python type appropriate to the given
            value_type. Strings are also converted to Unicode using
            ...
            To specify custom properties without specifying formatted
            value, one can pass None as the formatted value.

            One can also have a null-valued cell with formatted value
            and/or custom properties by specifying None for the value.

            This method ignores the custom properties except for
            checking that it is a dictionary. The custom properties
            are handled in the ToJSon and ToJSCode methods.

            The real type of the given value is not strictly
            checked. For example, any type can be used for string - as
            we simply take its str( ) and for boolean value we just
            check "if value".
        ...
        Raises:
            ...
            TableException: The value and type did not match in a
                            not-recoverable way, for example given
                            value 'abc' for type 'number'.
        """
        if isinstance(value, tuple):
            # In case of a tuple, we run the same function on the
            # value itself and add the formatted value.
        ...
        if value_type == "boolean":
            ...
            if isinstance(value, (six.integer_types, float)):
                return value
            raise TableException("Wrong type %s when expected number" %
                                 t_value)
            ...
            if isinstance(value, datetime.datetime):
                ...
                raise TableException("Wrong type %s when expected date" %
                                     t_value)
            ...
            if isinstance(value, datetime.datetime):
                ...
                raise TableException("Wrong type %s when expected time" %
                                     t_value)
    ...
    def EscapeForJSCode(encoder, value):
        if value is None:
            ...
            return "new Date(%d,%d,%d)" % (value.year, value.month - 1,
                                           value.day)
        else:
    ...
    def ColumnTypeParser(description):
        ...
        Returns:
            ...
            custom_properties where:
                ...
                - If custom properties are not given, an empty
            ...
            if len(description) > 2:
                ...
                if len(description) > 3:
                    ...
                    if len(description) > 4:
                        raise TableException("Description error: tuple of "
                                             "length > 4")
    ...
    def TableDescriptionParser(table_description, depth=0):
        ...
        Parses the user-submitted table description into an internal
        format used by the Python Table class. Returns the flat list
        of parsed columns.

        Args:

            table_description: A description of the table which should
                               comply with one of the formats
                               described below.

            depth: Optional. The depth of the first level in the
                   current description.  Used by recursive calls to
                   this function.

        Returns:

            List of columns, where each column represented by a
            dictionary with the keys: id, label, type, depth,
            container which means the following:

            - id: the id of the column

            - name: The name of the column

            - type: The datatype of the elements in this
                    column. Allowed types are described in
                    ColumnTypeParser().

            - depth: The depth of this column in the table description

            - container: 'dict', 'iter' or 'scalar' for parsing the
                         format easily.

            - custom_properties: The custom properties for this column.

            The returned description is flattened regardless of how it
            was given.
        ...
        Examples:
            ...
            output: [{'id': 'a', 'label': 'a', 'type': 'date', 'depth': 0,
                      'container': 'iter', 'custom_properties': {}},
                     {'id': 'b', 'label': 'b', 'type': 'timeofday', 'depth': 0,
                      'container': 'iter', 'custom_properties': {'foo': 'bar'}}
                    ]

            input: {'a': [('b', 'number'), ('c', 'string', 'column c')]}
            output: [{'id': 'a', 'label': 'a', 'type': 'string', 'depth': 0,
            ...
            input:  {('a', 'number', 'column a'): { 'b': 'number',
                                                    'c': 'string'}}
            ...
            output: [{'id': 'w', 'label': 'word', 'type': 'string', 'depth': 0,
                      'container': 'dict', 'custom_properties': {}},
            ...
            NOTE: there might be ambiguity in the case of a dictionary
            representation of a single column. For example, the
            following description can be parsed in 2 different ways:
            {'a': ('b', 'c')} can be thought of a single column with
            the id 'a', of type 'b' and the label 'c', or as 2
            columns: one named 'a', and the other named 'b' of type
            'c'. We choose the first option by default, and in case
            the second option is the right one, it is possible to make
            the key into a tuple (i.e. {('a',): ('b', 'c')}) or add
            more info into the tuple, thus making it look like this:
            {'a': ('b', 'c', 'b', {})} -- second 'b' is the label, and
            {} is the custom properties field.
        """
        # For the recursion step, we check for a scalar object (string
        # or tuple)
        ...
        # To differentiate between the two cases of more levels below
        # or this is the most inner dictionary, we consider the number
        # of keys (more then one key is indication for most inner
        # dictionary) and the type of the key and value in case of
        # only 1 key (if the type of key is string and the type of the
        # value is a tuple of 0-3 items, we assume this is the most
        # inner dictionary).
        # NOTE: this way of differentiating might create ambiguity. See docs.
        if (len(table_description) != 1 or
                (isinstance(list(table_description.keys())[0],
            ...
            columns = []
            # We sort the items, equivalent to sort the keys since
            # they are unique
            for key, value in sorted(table_description.items()):
                # We parse the column type as (key, type) or (key,
                # type, label) using ColumnTypeParser.
                if isinstance(value, tuple):
        ...
        return ([parsed_col] +
                Table.TableDescriptionParser(
                    list(table_description.values())[0],
                    depth=depth + 1))
    ...
    def NumberOfRows(self):
        """Returns the number of rows in the current data stored in
        the table."""
    ...
    def SetRowsCustomProperties(self, rows, custom_properties):
        ...
        Args:
            ...
            custom_properties: A string to string dictionary of custom
                               properties to set for all rows.
        """
    ...
    def LoadData(self, data, custom_properties=None):
        ...
        May also set the custom_properties for the added rows. The
        given custom properties dictionary specifies the dictionary
        that will be used for *all* given rows.

        Args:
            data: The rows that the table will contain.

            custom_properties: A dictionary of string to string to set
                               as the custom properties for all rows.
        """
        self.__data = []
    ...
    def AppendData(self, data, custom_properties=None):
        ...
        Data is appended in rows. Data must comply with the table
        schema passed in to __init__(). See CoerceValue() for a list
        of acceptable data types. See the class documentation for more
        information and examples of schema and data values.

        Args:

            data: The row to add to the table. The data must conform
                  to the table description format.

            custom_properties: A dictionary of string to string,
                               representing the custom properties to
                               add to all the rows.
    ...
    def _InnerAppendData(self, prev_col_values, data, col_index):
        ...
        if not data.keys():
            # In case this is an empty dictionary, we add a record
            # with the columns filled only until this point.
            self.__data.append(prev_col_values)
        else:
    ...
    def _PreparedData(self, order_by=()):
        ...
        Args:
            ...
            order_by: Optional. Specifies the name of the column(s) to
                      sort by, and (optionally) which direction to
                      sort in. Default sort direction is
                      asc. Following formats are accepted:

                      "string_col_name" -- For a single key in default
                      (asc) order.

                      ("string_col_name", "asc|desc") -- For a single key.

                      [("col_1","asc|desc"), ("col_2","asc|desc")] --
                      For more than one column, an array of tuples of
                      (col_name, "asc|desc").
        ...
        for key in reversed(order_by):
            if isinstance(key, six.string_types):
                key_fn = lambda row: row[0].get(key)
                reverse = False
    ...
    def ToJSCode(self, name, columns_order=None, order_by=()):
        ...
        Args:
            ...
            columns_order: Optional. Specifies the order of columns in
                           the output table. Specify a list of all
                           column IDs in the order in which you want
                           the table created.

                           Note that you must list all column IDs in
                           this parameter, if you use it.

            order_by: Optional. Specifies the name of the column(s) to sort by.
                      Passed as is to _PreparedData.

        Returns:
            A string of JS code that, when run, generates a Table with
            the given name and the data stored in the Table object.
    ...
    def ToHtml(self, columns_order=None, order_by=()):
        ...
        Args:
            ...
            columns_order: Optional. Specifies the order of columns in
                           the output table. Specify a list of all
                           column IDs in the order in which you want
                           the table created.  Note that you must list
                           all column IDs in this parameter, if you
                           use it.

            order_by: Optional. Specifies the name of the column(s) to
                      sort by.  Passed as is to _PreparedData.
        ...
        Raises:
            TableException: The data does not match the type.
        """
        table_template = ("<html><body><table border=\"1\">%s"
                          "</table></body></html>")
        ...
        for col in columns_order:
            columns_list.append(header_cell_template %
                                html_escape(col_dict[col]["label"]))
        ...
        for row, unused_cp in self._PreparedData(order_by):
            ...
            for col in columns_order:
                ...
                if isinstance(value, tuple):
                    ...
                    cells_list.append(cell_template %
                                      html_escape(self.ToString(value[1])))
                else:
                    cells_list.append(cell_template %
                                      html_escape(self.ToString(value)))
    ...
    def ToCsv(self, columns_order=None, order_by=(), separator=","):
        ...
        writer.writerow([csv_enc(col_dict[col]["label"])
                         for col in columns_order])
        ...
        for row, unused_cp in self._PreparedData(order_by):
            ...
            for col in columns_order:
                ...
                if col in row and row[col] is not None:
                    value = self.CoerceValue(row[col], col_dict[col]["type"])
                if isinstance(value, tuple):
                    # We have a formatted value. Using it only for
                    # date/time types.
                    if col_dict[col]["type"] in ["date", "datetime",
                                                 "timeofday"]:
                        cells_list.append(csv_enc(self.ToString(value[1])))
                    else:
                        cells_list.append(csv_enc(self.ToString(value[0])))
                else:
                    cells_list.append(csv_enc(self.ToString(value)))
            writer.writerow(cells_list)

        return csv_dec(csv_buffer.getvalue())
    ...
    def _ToJSonObj(self, columns_order=None, order_by=()):
        ...
        Args:
            ...
            columns_order: Optional. A list of all column IDs in the
                           order in which you want them created in the
                           output table. If specified, all column IDs
                           must be present.
        ...
        for row, cp in self._PreparedData(order_by):
            cell_objs = []
            for col in columns_order:
                value = self.CoerceValue(row.get(col, None),
                                         col_dict[col]["type"])
                if value is None:
                    cell_obj = None
    ...
    def ToJSon(self, columns_order=None, order_by=()):
        ...
        This method writes a JSON string that can be passed directly
        into a Google Visualization API Table constructor. Use this
        output if you are hosting the visualization HTML on your site,
        and want to code the data table in Python. Pass this string
        into the google.visualization.Table constructor, e.g,:
    ...
    def ToJSonResponse(self, columns_order=None, order_by=(), req_id=0,
                       response_handler=("google.visualization.Query"
                                         ".setResponse")):
        """Writes a table as a JSON response that can be returned
        as-is to a client.

        This method writes a JSON response to return to a client in
        response to a Google Visualization API query. This string can
        be processed by the calling page, and is used to deliver a
        data table to a visualization hosted on a different page.
        ...
        Args:
            ...
            response_handler: Optional. The response handler, as retrieved by
                              the request.
        ...
        Note: The URL returning this string can be used as a data source by
              Google Visualization Gadgets or from JS code.
        """
    ...
    def ToResponse(self, columns_order=None, order_by=(), tqx=""):
        """Writes the right response according to the request string
        passed in tqx.

        This method parses the tqx request string (format of which is
        defined in the documentation for implementing a data source of
        Google Visualization), and returns the right response
        according to the request.

        It parses out the "out" parameter of tqx, calls the relevant
        response (ToJSonResponse() for "json", ToCsv() for "csv",
        ToHtml() for "html", ToTsvExcel() for "tsv-excel") and passes
        the response function the rest of the relevant request keys.

        Args:

            columns_order: Optional. Passed as is to the relevant
                           response function.

            order_by: Optional. Passed as is to the relevant response function.

            tqx: Optional. The request string as received by HTTP
                 GET. Should be in the format
                 "key1:value1;key2:value2...". All keys have a default
                 value, so an empty string will just do the default
                 (which is calling ToJSonResponse() with no extra
                 parameters).
        ...
        Raises:
            ...
            TableException: One of the parameters passed in tqx is not
                            supported.
        """
        tqx_dict = {}
        if tqx:
